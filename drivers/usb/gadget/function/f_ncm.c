...
#define FORMATS_SUPPORTED                                                      \
	(USB_CDC_NCM_NTB16_SUPPORTED | USB_CDC_NCM_NTB32_SUPPORTED)
...
#define INIT_NDP16_OPTS                                                        \
	{                                                                      \
		...
		.ndplen_align = 4, .dgram_item_len = 1, .block_length = 1,     \
		.ndp_index = 1, .reserved1 = 0, .reserved2 = 0,                \
		.next_ndp_index = 1,                                           \
	}
...
#define INIT_NDP32_OPTS                                                        \
	{                                                                      \
		...
		.ndplen_align = 8, .dgram_item_len = 2, .block_length = 2,     \
		.ndp_index = 2, .reserved1 = 1, .reserved2 = 2,                \
		.next_ndp_index = 2,                                           \
	}
...
static struct sk_buff *package_for_tx(struct f_ncm *ncm)
{
	...
	ndp_pad =
		ALIGN(ncm->skb_tx_data->len, ndp_align) - ncm->skb_tx_data->len;
	...
	if (skb) {
		...
		if (ncm->skb_tx_data &&
		    (ncm->ndp_dgram_count >= TX_MAX_NUM_DPE ||
		     (ncm->skb_tx_data->len + div + rem + skb->len +
		...
		if (!ncm->skb_tx_data) {
			 ...
			 * 16k packet.
			 */
			ncm->skb_tx_ndp = alloc_skb(
				(int)(opts->ndp_size +
				      opts->dpe_size * TX_MAX_NUM_DPE),
				GFP_ATOMIC);
			if (!ncm->skb_tx_ndp)
				goto err;
			  ...
			  struct sk_buff_head *list)
{
	...
	do {
		 ...
		 */
		if ((ndp_len <
		     opts->ndp_size + 2 * 2 * (opts->dgram_item_len * 2)) ||
		...
		do {
			...
			if (ncm->is_crc) {
				...
				crc = get_unaligned_le32(skb->data + index +
							 dg_len - crc_len);
				crc2 = ~crc32_le(~0, skb->data + index,
						 dg_len - crc_len);
				if (crc != crc2) {
					INFO(port->func.config->cdev,
					     "Bad CRC\n");
			...
			if (skb2 == NULL)
				goto err;
			skb_put_data(skb2, skb->data + index, dg_len - crc_len);
...
static int ncm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (cdev->use_os_string) {
		f->os_desc_table =
			kzalloc(sizeof(*f->os_desc_table), GFP_KERNEL);
		if (!f->os_desc_table)
			return -ENOMEM;
		f->os_desc_n = 1;
	...
	DBG(cdev, "CDC Network: %s speed IN/%s OUT/%s NOTIFY/%s\n",
	    ...
	    gadget_is_dualspeed(c->cdev->gadget)  ? "dual" :
						    "full",
	    ncm->port.in_ep->name, ncm->port.out_ep->name, ncm->notify->name);
	return 0;
...
static void ncm_setup_work(struct work_struct *data)
{
	...
	snprintf(mirror_link_string, MIRROR_LINK_STRING_LENGTH_MAX,
		 "MirrorLink=V%d.%d", _ncm_setup_desc->major,
		 _ncm_setup_desc->minor);
...
static void ncm_unbind(struct usb_configuration *c, struct usb_function *f)
{
	struct f_ncm *ncm = func_to_ncm(f);
	struct f_ncm_opts *opts =
		container_of(f->fi, struct f_ncm_opts, func_inst);
