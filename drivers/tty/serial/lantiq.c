...
static int lqasc_rx_chars(struct uart_port *port)
{
	...
	while (fifocnt--) {
		...
		rsr = (readl(port->membase + LTQ_ASC_STATE) & ASCSTATE_ANY) |
		      UART_DUMMY_UER_RX;
...
static void lqasc_tx_chars(struct uart_port *port)
{
	...
	while (((readl(port->membase + LTQ_ASC_FSTAT) & ASCFSTAT_TXFREEMASK) >>
		ASCFSTAT_TXFREEOFF) != 0) {
		if (port->x_char) {
...
static int lqasc_startup(struct uart_port *port)
{
	...
	retval = request_irq(ltq_port->rx_irq, lqasc_rx_int, 0, "asc_rx", port);
	if (retval) {
		pr_err("failed to request lqasc_rx_int\n");
		goto err1;
	}

	retval = request_irq(ltq_port->err_irq, lqasc_err_int, 0, "asc_err",
			     port);
	if (retval) {
...
static void lqasc_set_termios(struct uart_port *port, struct ktermios *new,
			      struct ktermios *old)
{
	unsigned int cflag;
	unsigned int iflag;
...
static int lqasc_request_port(struct uart_port *port)
{
	...
	res = devm_request_mem_region(&pdev->dev, res->start, size,
				      dev_name(&pdev->dev));
	if (!res) {
	...
	if (port->flags & UPF_IOREMAP) {
		port->membase =
			devm_ioremap_nocache(&pdev->dev, port->mapbase, size);
...
static void lqasc_serial_early_console_write(struct console *co, const char *s,
					     u_int count)
{
...
static int __init lqasc_serial_early_console_setup(
...
static int __init lqasc_probe(struct platform_device *pdev)
{
	...
	port->ops = &lqasc_pops;
	port->fifosize = 16;
	port->type = PORT_LTQ_ASC, port->line = line;
	port->dev = &pdev->dev;
