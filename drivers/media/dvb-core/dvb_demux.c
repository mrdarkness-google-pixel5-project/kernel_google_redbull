...
int dvb_dmx_video_pattern_search(
	...
	int patterns_num, const u8 *buf, size_t buf_size,
					   ...
					   const u8 *buf)
{
						  ...
						  struct dvb_demux_filter * f)
	{
		...
		return feed->cb.sec(feed->feed.sec.secbuf,
				    feed->feed.sec.seclen, NULL, 0, &f->filter,
				    &feed->buffer_flags);
	}

	static inline int dvb_dmx_swfilter_section_feed(struct dvb_demux_feed *
							feed)
	{
		...
		if (sec->check_crc) {
			...
			if (section_syntax_indicator &&
			    demux->check_crc32(feed, sec->secbuf,
					       sec->seclen)) {
				if (dvb_demux_performancecheck)
					demux->total_crc_time +=
						dvb_dmx_calc_time_delta(
							pre_crc_time);

				/* Notify on CRC error */
				feed->cb.sec(NULL, 0, NULL, 0, &f->filter,
					     &feed->buffer_flags);
 ...
 */
	static int dvb_dmx_swfilter_section_copy_dump(
		struct dvb_demux_feed * feed, const u8 *buf, u8 len)
	{
		struct dvb_demux *demux = feed->demux;
		struct dmx_section_feed *sec = &feed->feed.sec;
		u16 limit, seclen, n;
	...
	static int dvb_dmx_swfilter_section_one_packet(
		struct dvb_demux_feed * feed, const u8 *buf)
	{
		u8 p, count;
		int ccok, dc_i = 0;
		u8 cc;
...
>>>>>>> 0ec5f56
		...
		if (buf[1] & 0x40) {
			...
			dvb_dmx_swfilter_section_copy_dump(feed, &buf[p],
							   count);
		}
 ...
 */
	...
	static int dvb_demux_save_idx_event(
		struct dvb_demux_feed * feed,
		struct dmx_index_event_info * idx_event, int traverse_from_tail)
	{
		...
		idx_entry =
			list_first_entry(&feed->rec_info->idx_info.free_list,
	...
	static inline void dvb_dmx_notify_indexing(struct dvb_demux_feed * feed)
	{
		...
		list_for_each_safe (pos, n,
				    &feed->rec_info->idx_info.ready_list) {
			curr_entry =
				list_entry(pos, struct dmx_index_entry, next);

			if ((feed->rec_info->idx_info.min_pattern_tsp_num ==
			     (u64)-1) ||
	...
	static void dvb_dmx_process_pattern_result(
		struct dvb_demux_feed * feed,
		struct dvb_dmx_video_patterns_results * patterns, int pattern,
		u64 curr_stc, u64 prev_stc, u64 curr_match_tsp,
		u64 prev_match_tsp, u64 curr_pusi_tsp, u64 prev_pusi_tsp)
	{
		...
		h264_frame_start =
			patterns->info[pattern].type &
			(DMX_IDX_H264_IDR_START | DMX_IDX_H264_NON_IDR_START);

		vc1_frame_start =
			patterns->info[pattern].type & DMX_IDX_VC1_FRAME_START;

		if (!mpeg_frame_start && !h264_frame_start &&
		    !vc1_frame_start) {
	...
	{
		spin_lock(&feed->demux->lock);
		dvb_dmx_process_pattern_result(feed, patterns, pattern,
					       curr_stc, prev_stc,
	...
	static void dvb_dmx_index(struct dvb_demux_feed * feed, const u8 *buf,
	 ...
	 */
		...
		if ((feed->idx_params.types & DMX_IDX_RAI) && /* index RAI? */
			...
			idx_event.last_pusi_tsp_num = feed->curr_pusi_tsp_num;
			dvb_demux_save_idx_event(feed, &idx_event, 1);
		}
	 ...
	 */
		...
		found_patterns = dvb_dmx_video_pattern_search(
			feed->patterns, feed->pattern_num, &buf[p], count,
			&feed->prefix_size, &patterns);

		for (i = 0; i < found_patterns; i++)
			dvb_dmx_process_pattern_result(
				feed, &patterns, i, stc, feed->prev_stc,
				feed->rec_info->ts_output_count,
				feed->prev_tsp_num, feed->curr_pusi_tsp_num,
				feed->prev_pusi_tsp_num);
	 ...
	 */
		if (feed->tsp_out_format == DMX_TSP_FORMAT_192_HEAD)
			feed->cb.ts(timestamp, TIMESTAMP_LEN, NULL, 0,
	...
	static inline void dvb_dmx_configure_decoder_fullness(
		struct dvb_demux * demux, int initialize)
	{
		struct dvb_demux_feed *feed;
		int j;
	...
	static inline int dvb_dmx_swfilter_buffer_check(
		struct dvb_demux * demux, u16 pid)
	{
		int desired_space;
		int ret;
	 ...
	 */
		for (j = 0; j < demux->feednum; j++) {
			...
			/* else - section case */
			desired_space =
				feed->feed.sec.tsfeedp + 188; /* upper bound */
	...
	static inline void dvb_dmx_swfilter_packet_type(
		struct dvb_demux_feed * feed, const u8 *buf,
		const u8 timestamp[TIMESTAMP_LEN])
	{
		u16 pid = ts_pid(buf);
	 ...
	 */
		if ((scrambling_bits && !feed->scrambling_bits) ||
			...
			dmx_data_ready.scrambling_bits.new_value =
				scrambling_bits;

			if (feed->type == DMX_TYPE_SEC)
				dvb_dmx_notify_section_event(
					feed, &dmx_data_ready, 0);
		...
		case DMX_TYPE_TS:
			...
			if (feed->ts_type & TS_PACKET) {
				if (feed->ts_type & TS_PAYLOAD_ONLY) {
					if (!feed->secure_mode.is_secured)
						dvb_dmx_swfilter_payload(feed,
									 buf);
				} else {
					dvb_dmx_swfilter_output_packet(
						feed, buf, timestamp);
				}
			}
			...
			if ((feed->ts_type & TS_DECODER) &&
				...
				if (feed->demux->write_to_decoder)
					feed->demux->write_to_decoder(feed, buf,
								      188);
			break;
		...
		case DMX_TYPE_SEC:
			...
			if (dvb_dmx_swfilter_section_one_packet(feed, buf) < 0)
				feed->feed.sec.seclen = feed->feed.sec.secbufp =
					0;
			break;
...
#define DVR_FEED(f)                                                            \
						  ...
						  const u8 *buf)
	{
		...
		if (pid < MAX_PID) {
			...
			if ((buf[3] & 0xf) != demux->cnt_storage[pid]) {
				list_for_each_entry (feed, &demux->feed_list,
						     list_head) {
					if ((feed->pid != pid) &&
					    (feed->pid != 0x2000))
						continue;
				...
				dprintk_tscheck(
					"TS packet counter mismatch contd\n");
				dprintk_tscheck(
	...
	static void dvb_dmx_swfilter_one_packet(
		...
		if (dvb_demux_speedcheck) {
			...
			if (!(demux->speed_pkts_cnt % SPEED_PKTS_INTERVAL)) {
				...
				if (ktime_to_ns(demux->speed_last_time) != 0) {
					speed_bytes =
						(u64)demux->speed_pkts_cnt *
						188 * 8;
					/* convert to 1024 basis */
					speed_bytes =
						1000 *
						div64_u64(speed_bytes, 1024);
					speed_timedelta = ktime_ms_delta(
						cur_time,
				      ...
				      size_t count)
	{
		...
		while (count--) {
			if (buf[0] == 0x47)
				dvb_dmx_swfilter_one_packet(demux, buf,
							    timestamp);
			buf += 188;
		}
		...
		if (dvb_demux_performancecheck)
			demux->total_process_time +=
				dvb_dmx_calc_time_delta(pre_time);
	}
	EXPORT_SYMBOL(dvb_dmx_swfilter_packets);

	static inline int find_next_packet(const u8 *buf, int pos, size_t count,
					   const int pktsize,
					   const int leadingbytes)
	{
		int start = pos, lost;
	...
	static inline void _dvb_dmx_swfilter(struct dvb_demux * demux,
					     ...
					     const int leadingbytes)
	{
		...
		if (demux->tsbufp) { /* tsbuf[0] is now 0x47. */
			...
			if (pktsize == 192 && leadingbytes &&
			    demux->tsbuf[leadingbytes] ==
				    0x47) /* double check */
				dvb_dmx_swfilter_one_packet(
					demux, demux->tsbuf + TIMESTAMP_LEN,
					timestamp);
			else if (demux->tsbuf[0] == 0x47) /* double check */
				dvb_dmx_swfilter_one_packet(demux, demux->tsbuf,
							    timestamp);
			demux->tsbufp = 0;
			p += j;
		}
		...
		while (1) {
			...
			if (pktsize == 192) {
				if (leadingbytes) {
					...
					memcpy(timestamp, &buf[p],
					       TIMESTAMP_LEN);
				} else {
					memcpy(timestamp, &buf[p + 188],
					       TIMESTAMP_LEN);
				}
			} else {
				memset(timestamp, 0, TIMESTAMP_LEN);
	...
	void dvb_dmx_swfilter_format(struct dvb_demux * demux, const u8 *buf,
				     size_t count,
				     enum dmx_tsp_format_t tsp_format)
	{
		...
		case DMX_TSP_FORMAT_192_HEAD:
			_dvb_dmx_swfilter(demux, buf, count, 192,
					  TIMESTAMP_LEN);
			break;
	...
	static struct dvb_demux_feed *dvb_dmx_feed_alloc(struct dvb_demux *
							 demux)
	{
		int i;
	...
	const struct dvb_dmx_video_patterns *dvb_dmx_get_pattern(
		u64 dmx_idx_pattern)
	{
	...
	static void dvb_dmx_init_idx_state(struct dvb_demux_feed * feed)
	{
		...
		if ((feed->pattern_num < DVB_DMX_MAX_SEARCH_PATTERN_NUM) &&
		    (feed->idx_params.types &
		     (DMX_IDX_H264_SPS | DMX_IDX_H264_FIRST_SPS_FRAME_START |
		      DMX_IDX_H264_FIRST_SPS_FRAME_END))) {
			feed->patterns[feed->pattern_num] =
		...
		if ((feed->pattern_num < DVB_DMX_MAX_SEARCH_PATTERN_NUM) &&
			...
			feed->patterns[feed->pattern_num] = dvb_dmx_get_pattern(
				DMX_IDX_H264_ACCESS_UNIT_DEL);
			feed->pattern_num++;
		}
	...
	static int dmx_ts_feed_set(struct dmx_ts_feed * ts_feed, u16 pid,
				   int ts_type, enum dmx_ts_pes pes_type,
	...
	static int dmx_ts_feed_stop_filtering(struct dmx_ts_feed * ts_feed)
	{
		...
		if (feed->rec_info) {
			if (feed->pattern_num)
				feed->rec_info->idx_info
					.pattern_search_feeds_num--;
			if (feed->idx_params.enable)
				feed->rec_info->idx_info.indexing_feeds_num--;
			dvb_dmx_free_rec_info(ts_feed);
			feed->rec_info = NULL;
	...
	static int dmx_ts_feed_reuse_decoder_buffer(
		struct dmx_ts_feed * ts_feed, int cookie)
	{
		struct dvb_demux_feed *feed = (struct dvb_demux_feed *)ts_feed;
		struct dvb_demux *demux = feed->demux;
		int ret;
	...
	static int dmx_ts_set_video_codec(struct dmx_ts_feed * ts_feed,
						 ...
						 idx_params)
	{
		...
		if ((feed->state == DMX_STATE_GO) && !feed->rec_info) {
			mutex_unlock(&dvbdmx->mutex);
			return -EINVAL;
		}
	...
	static int dvbdmx_ts_insertion_insert_buffer(
		struct dmx_ts_feed * ts_feed, char *data, size_t size)
	{
		struct dvb_demux_feed *feed = (struct dvb_demux_feed *)ts_feed;
		struct dvb_demux *demux = feed->demux;
	...
	static int dmx_ts_set_tsp_out_format(struct dmx_ts_feed * ts_feed,
					     enum dmx_tsp_format_t tsp_format)
	{
		struct dvb_demux_feed *feed = (struct dvb_demux_feed *)ts_feed;
		struct dvb_demux *dvbdmx = feed->demux;
 ...
 */
	...
	static int dvbdmx_ts_flush_buffer(struct dmx_ts_feed * ts_feed,
					  size_t length)
	{
		struct dvb_demux_feed *feed = (struct dvb_demux_feed *)ts_feed;
		struct dvb_demux *demux = feed->demux;
	 ...
	 */
		...
		(*ts_feed)->get_decoder_buff_status =
			dmx_ts_feed_decoder_buff_status;
		(*ts_feed)->reuse_decoder_buffer =
			dmx_ts_feed_reuse_decoder_buffer;
 ...
 ******************************************************************************/
	...
	static int dmx_section_feed_allocate_filter(
	...
	static int dmx_section_feed_set(struct dmx_section_feed * feed, u16 pid,
	...
	static int dmx_section_feed_start_filtering(struct dmx_section_feed *
						    feed)
	{
		struct dvb_demux_feed *dvbdmxfeed =
			(struct dvb_demux_feed *)feed;
	...
	static int dmx_section_feed_stop_filtering(struct dmx_section_feed *
						   feed)
	{
		struct dvb_demux_feed *dvbdmxfeed =
			(struct dvb_demux_feed *)feed;
	...
	static int dmx_section_feed_data_ready_cb(
		struct dmx_section_feed * feed,
	...
	static int dmx_section_feed_release_filter(
		struct dmx_section_feed * feed,
		struct dmx_section_filter * filter)
	{
		struct dvb_demux_filter *dvbdmxfilter =
	...
	static int dvbdmx_section_feed_oob_cmd(struct dmx_section_feed *
						       section_feed,
					       struct dmx_oob_command * cmd)
	{
		struct dvb_demux_feed *feed =
			(struct dvb_demux_feed *)section_feed;
		struct dvb_demux *dvbdmx = feed->demux;
		struct dmx_data_ready data;
		int ret = 0;
						...
						dmx_section_cb callback)
	{
		...
		(*feed)->get_scrambling_bits =
			dvbdmx_section_get_scrambling_bits;
 ...
 ******************************************************************************/
				...
				const char __user *buf, size_t count)
	{
		...
		dvb_dmx_swfilter_format(dvbdemux, buf, count,
					dvbdemux->tsp_format);
	...
	static int dvbdmx_set_playback_mode(
		struct dmx_demux * demux, enum dmx_playback_mode_t mode,
	...
	static int dvbdmx_set_tsp_format(struct dmx_demux * demux,
					 enum dmx_tsp_format_t tsp_format)
	{
		struct dvb_demux *dvbdemux = (struct dvb_demux *)demux;
	...
	int dvb_dmx_init(struct dvb_demux * dvbdemux)
	{
		...
		dvbdemux->users = 0;
		dvbdemux->filter = vmalloc(array_size(
			sizeof(struct dvb_demux_filter), dvbdemux->filternum));

		if (!dvbdemux->filter)
			return -ENOMEM;

		dvbdemux->feed = vmalloc(array_size(
			sizeof(struct dvb_demux_feed), dvbdemux->feednum));
		if (!dvbdemux->feed) {
		...
		dvbdemux->rec_info_pool = vmalloc(array_size(
			sizeof(struct dvb_demux_rec_info), dvbdemux->feednum));
		...
		snprintf(dvbdemux->alias, MAX_DVB_DEMUX_NAME_LEN, "demux%d",
			 dvb_demux_index++);
		...
		if (dvbdemux->dmx.debugfs_demux_dir != NULL) {
			debugfs_create_u32("total_processing_time", 0664,
					   dvbdemux->dmx.debugfs_demux_dir,
					   &dvbdemux->total_process_time);

			debugfs_create_u32("total_crc_time", 0664,
					   dvbdemux->dmx.debugfs_demux_dir,
