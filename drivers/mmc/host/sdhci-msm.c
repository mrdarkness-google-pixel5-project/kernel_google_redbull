...
MODULE_PARM_DESC(
	bus_mode,
	"Set this parameter during bootup to set the desired bus speed mode for eMMC only.");
...
static inline int msm_dll_poll_ck_out_en(struct sdhci_host *host, u8 poll)
{
	...
	ck_out_en = !!(
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_CONFIG) &
		CORE_CK_OUT_EN);
...
static int msm_enable_cdr_cm_sdc4_dll(struct sdhci_host *host)
{
	...
	config = readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_CONFIG);
	config |= CORE_CDR_EN;
...
static ssize_t store_auto_cmd21(struct device *dev,
			   ...
			   enum dll_init_context init_context)
{
	...
	if (msm_host->use_7nm_dll) {
		if (msm_host->dll_hsr) {
			writel_relaxed(
				msm_host->dll_hsr->dll_usr_ctl,
				host->ioaddr +
					msm_host_offset->CORE_DLL_USR_CTL);
			writel_relaxed(
				msm_host->dll_hsr->dll_config_3,
				host->ioaddr +
					msm_host_offset->CORE_DLL_CONFIG_3);
		} else {
			writel_relaxed(
				DLL_USR_CTL_POR_VAL | FINE_TUNE_MODE_EN |
					ENABLE_DLL_LOCK_STATUS | BIAS_OK_SIGNAL,
				host->ioaddr +
					msm_host_offset->CORE_DLL_USR_CTL);

			writel_relaxed(
				DLL_CONFIG_3_POR_VAL,
				host->ioaddr +
					msm_host_offset->CORE_DLL_CONFIG_3);
		}
	}
	...
	if (msm_host->mmc->card && !(msm_host->enhanced_strobe &&
...
static int sdhci_msm_cdclp533_calibration(struct sdhci_host *host)
{
	...
	ret = readl_poll_timeout(host->ioaddr + CORE_CSR_CDC_STATUS0,
				 calib_done,
				 (calib_done & CORE_CALIBRATION_DONE), 1, 50);
...
static int sdhci_msm_cm_dll_sdc4_calibration(struct sdhci_host *host)
{
	...
	if (msm_host->dll_hsr && msm_host->dll_hsr->ddr_config) {
		writel_relaxed(msm_host->dll_hsr->ddr_config,
			       host->ioaddr + msm_host_offset->CORE_DDR_CONFIG);
	} else if (msm_host->rclk_delay_fix) {
		writel_relaxed(DDR_CONFIG_POR_VAL,
			       host->ioaddr + msm_host_offset->CORE_DDR_CONFIG);
	} else {
		writel_relaxed(DDR_CONFIG_POR_VAL,
			       host->ioaddr +
	...
	ret = readl_poll_timeout(
		host->ioaddr + msm_host_offset->CORE_DLL_STATUS, dll_status,
		(dll_status & CORE_DDR_DLL_LOCK), 10, 1000);
 ...
 */
void sdhci_msm_mm_dbg_configure(struct sdhci_host *host, u32 mask, u32 match,
				u32 bit_shift, u32 testbus)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	...
	/* Enable debug mode */
	writel_relaxed(ENABLE_DBG, host->ioaddr + SDCC_TESTBUS_CONFIG);
	writel_relaxed(DUMMY, host->ioaddr + SDCC_DEBUG_EN_DIS_REG);
	writel_relaxed((readl_relaxed(host->ioaddr + SDCC_TESTBUS_CONFIG) |
			TESTBUS_EN),
	...
	writel_relaxed((testbus & TB_CONF_MASK),
		       host->ioaddr + SDCC_DEBUG_MM_TB_CFG_REG);
	/* Initiate conf shifting */
	writel_relaxed(BIT(8), host->ioaddr + SDCC_DEBUG_MM_TB_CFG_REG);

	/* Wait for test bus to be configured */
	ret = readl_poll_timeout(host->ioaddr + SDCC_DEBUG_MM_TB_CFG_REG, val,
				 !(val & TB_WRITE_STATUS), 50, 1000);
	if (ret == -ETIMEDOUT)
...
void sdhci_msm_enter_dbg_mode(struct sdhci_host *host)
{
	...
	if (msm_host->minor >= 2)
		enable_dbg_feature |= FSM_HISTORY | AUTO_RECOVERY_DISABLE |
				      MM_TRIGGER_DISABLE | IIB_EN;
...
void sdhci_msm_exit_dbg_mode(struct sdhci_host *host)
{
	...
	/* Exit debug mode */
	writel_relaxed(DISABLE_DBG, host->ioaddr + SDCC_TESTBUS_CONFIG);
	writel_relaxed(DUMMY, host->ioaddr + SDCC_DEBUG_EN_DIS_REG);
...
retry:
		...
		if (card && (cmd.error || data.error)) {
			...
			while (sts_retry) {
				...
				if (sts_cmd.error ||
					...
					pr_debug(
						"%s: phase %d sts cmd err %d resp 0x%x\n",
	...
	if ((tuned_phase_cnt == NUM_TUNING_PHASES) && card &&
	    mmc_card_mmc(card)) {
		/*
					...
					struct sdhci_msm_pltfm_data *pdata)
{
	...
	pctrl_data->pins_active =
		pinctrl_lookup_state(pctrl_data->pctrl, "active");
	if (IS_ERR(pctrl_data->pins_active)) {
		ret = PTR_ERR(pctrl_data->pins_active);
		dev_err(dev, "Could not get active pinstates, err:%d\n", ret);
		goto out;
	}
	pctrl_data->pins_sleep =
		pinctrl_lookup_state(pctrl_data->pctrl, "sleep");
	...
	pctrl_data->pins_drv_type_400KHz =
		pinctrl_lookup_state(pctrl_data->pctrl, "ds_400KHz");
	...
	pctrl_data->pins_drv_type_50MHz =
		pinctrl_lookup_state(pctrl_data->pctrl, "ds_50MHz");
	if (IS_ERR(pctrl_data->pins_drv_type_50MHz)) {
		dev_dbg(dev, "Could not get 50M pinstates, err:%d\n", ret);
		pctrl_data->pins_drv_type_50MHz = NULL;
	}

	pctrl_data->pins_drv_type_100MHz =
		pinctrl_lookup_state(pctrl_data->pctrl, "ds_100MHz");
	...
	pctrl_data->pins_drv_type_200MHz =
		pinctrl_lookup_state(pctrl_data->pctrl, "ds_200MHz");
					...
					struct sdhci_msm_pltfm_data *pdata)
{
	...
	if (!ret) {
		...
		dev_err(dev,
			"Parsing Pinctrl failed with %d, falling back on GPIO lib\n",
			ret);
		ret = 0;
	}
	...
	if (cnt > 0) {
		pin_data->gpio_data = devm_kzalloc(
			dev, sizeof(struct sdhci_msm_gpio_data), GFP_KERNEL);
		if (!pin_data->gpio_data) {
			ret = -ENOMEM;
			goto out;
		}
		pin_data->gpio_data->size = cnt;
		pin_data->gpio_data->gpio = devm_kzalloc(
			dev, cnt * sizeof(struct sdhci_msm_gpio), GFP_KERNEL);
		...
		for (i = 0; i < cnt; i++) {
			...
			of_property_read_string_index(np, "qcom,gpio-names", i,
						      &name);
				      ...
				      struct sdhci_msm_pltfm_data *pdata)
{
	...
	if (of_property_count_u32_elems(np, "qcom,pm-qos-irq-latency") !=
		...
		dev_err(dev,
			"%s: could not read %d values for 'qcom,pm-qos-irq-latency'\n",
					     ...
					     struct sdhci_msm_pltfm_data *pdata)
{
	...
	for (i = 0; i < nr_groups; i++) {
		of_property_read_u32_index(np, "qcom,pm-qos-cpu-groups", i,
					   &mask);
...
static int
sdhci_msm_pm_qos_parse_latency(struct device *dev, const char *name,
			       int nr_groups,
			       struct sdhci_msm_pm_qos_latency **latency)
{
	...
	if (ret > 0 && ret != SDHCI_POWER_POLICY_NUM * nr_groups) {
		dev_err(dev,
			"%s: invalid number of values for property %s: expected=%d actual=%d\n",
				   ...
				   struct sdhci_msm_pltfm_data *pdata)
{
	...
	if (!sdhci_msm_pm_qos_parse_cpu_groups(dev, pdata)) {
		pdata->pm_qos_data.cmdq_valid = !sdhci_msm_pm_qos_parse_latency(
			dev, "qcom,pm-qos-cmdq-latency-us",
			pdata->pm_qos_data.cpu_group_map.nr_groups,
			&pdata->pm_qos_data.cmdq_latency);
		pdata->pm_qos_data.legacy_valid =
			!sdhci_msm_pm_qos_parse_latency(
				dev, "qcom,pm-qos-legacy-latency-us",
				pdata->pm_qos_data.cpu_group_map.nr_groups,
		...
		if (!pdata->pm_qos_data.cmdq_valid &&
			...
			dev_err(dev,
				"%s: invalid PM QoS latency values. Voting for cpu group will be disabled\n",
				__func__);
		}
	} else {
		dev_notice(dev,
			   "%s: PM QoS voting for cpu group will be disabled\n",
			   __func__);
	}
}
...

{
	...
	if (sdhci_msm_dt_get_array(dev, "qcom,dll-hsr-list", &dll_hsr_table,
				   &dll_hsr_table_len, 0))
		goto skip_hsr;
			       ...
			       struct sdhci_msm_host *msm_host)
{
	...
	msm_host->core_reset = devm_reset_control_get(dev, "core_reset");
					  ...
					  struct sdhci_msm_pltfm_data *pdata)
{
	...
	pdata->vreg_data = devm_kzalloc(
		dev, sizeof(struct sdhci_msm_slot_reg_data), GFP_KERNEL);
	if (!pdata->vreg_data) {
	...
	if (sdhci_msm_dt_parse_vreg_info(dev, &pdata->vreg_data->vdd_io_data,
					 "vdd-io")) {
...
sdhci_msm_populate_pdata(struct device *dev, struct sdhci_msm_host *msm_host)
{
	...
	if (sdhci_msm_dt_get_array(dev, "qcom,clk-rates", &clk_table,
				   &clk_table_len, 0)) {
	...
	if (!sdhci_msm_dt_get_array(dev, "qcom,ice-clk-rates", &ice_clk_table,
				    &ice_clk_table_len, 0)) {
	...
	if (sdhci_msm_dt_get_array(
		    dev, "qcom,devfreq,freq-table",
	...
	for (i = 0; i < len; i++) {
		...
		of_property_read_string_index(np, "qcom,bus-speed-mode", i,
					      &name);
		if (!name)
			continue;
				  ...
				  struct platform_device *pdev)
{
	...
	data = devm_kzalloc(dev, sizeof(struct sdhci_msm_bus_voting_data),
			    GFP_KERNEL);
	if (!data) {
		rc = -ENOMEM;
		goto out;
	...
	if (data->bus_pdata) {
		rc = sdhci_msm_dt_get_array(dev, "qcom,bus-bw-vectors-bps",
					    &data->bw_vecs, &data->bw_vecs_size,
					    0);
		if (rc) {
			dev_err(&pdev->dev,
	...
	if (host->pdata->voting_data && host->pdata->voting_data->bus_pdata &&
...
static int sdhci_msm_vreg_set_optimum_mode(struct sdhci_msm_reg_data *vreg,
					   int uA_load)
{
	int ret = 0;
...
static int sdhci_msm_setup_vreg(struct sdhci_msm_pltfm_data *pdata, bool enable,
				bool is_init)
{
	int ret = 0, i;
	...
	if (!curr_slot) {
		pr_debug(
			"%s: vreg info unavailable,assuming the slot is powered by always on domain\n",
			__func__);
		goto out;
	}
					      ...
					      bool enable)
{
	...
	if (enable && !msm_host->is_sdiowakeup_enabled)
		...
		dev_warn(&msm_host->pdev->dev,
			 "%s: wakeup to config: %d curr: %d\n", __func__,
			 enable, msm_host->is_sdiowakeup_enabled);
...
void sdhci_msm_dump_pwr_ctrl_regs(struct sdhci_host *host)
{
	...
	pr_err("%s: PWRCTL_STATUS: 0x%08x | PWRCTL_MASK: 0x%08x | PWRCTL_CTL: 0x%08x, pwr isr state=0x%x\n",
	       ...
	       sdhci_msm_readl_relaxed(host, msm_host_offset->CORE_PWRCTL_MASK),
	       sdhci_msm_readl_relaxed(host, msm_host_offset->CORE_PWRCTL_CTL),
	       irq_flags);
	...
	mmc_log_string(
		...
		sdhci_msm_readb_relaxed(host, msm_host_offset->CORE_PWRCTL_CTL),
		irq_flags);
}
...
static int sdhci_msm_clear_pwrctl_status(struct sdhci_host *host, u8 value)
{
	...
	do {
		...
		retry--;
		udelay(10);
	} while (value & sdhci_msm_readb_relaxed(
				 host, msm_host_offset->CORE_PWRCTL_STATUS));
...
static void sdhci_msm_vbias_bypass_wa(struct sdhci_host *host)
{
	...
	config =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC);
	/*
...
static irqreturn_t sdhci_msm_pwr_irq(int irq, void *data)
{
	...
	irq_status = sdhci_msm_readb_relaxed(
		host, msm_host_offset->CORE_PWRCTL_STATUS);
	...
	if (irq_status & CORE_PWRCTL_BUS_OFF) {
		if (msm_host->pltfm_init_done)
			ret = sdhci_msm_setup_vreg(msm_host->pdata, false,
						   false);
		if (!ret) {
...
static ssize_t show_sdhci_max_bus_bw(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
...
static ssize_t store_sdhci_max_bus_bw(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
...
static void sdhci_msm_check_power_status(struct sdhci_host *host, u32 req_type)
{
	...
	if (!msm_host->mci_removed)
		io_sig_sts = sdhci_msm_readl_relaxed(
			host, msm_host_offset->CORE_GENERICS);
	...
	if (req_type & (REQ_IO_HIGH | REQ_IO_LOW)) {
		if (!(io_sig_sts & SWITCHABLE_SIGNALLING_VOL) ||
			...
			pr_debug(
				"%s: do not wait for power IRQ that never comes\n",
				mmc_hostname(host->mmc));
			spin_unlock_irqrestore(&host->lock, flags);
			return;
		}
...
static void sdhci_msm_registers_save(struct sdhci_host *host)
{
	...
	if (!msm_host->regs_restore.is_supported && !msm_host->reg_store)
		return;

	msm_host->regs_restore.vendor_func =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC);
	msm_host->regs_restore.vendor_pwrctl_mask =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_PWRCTL_MASK);
	msm_host->regs_restore.vendor_func2 = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC_FUNC2);
	msm_host->regs_restore.vendor_func3 = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC3);
	...
	msm_host->regs_restore.vendor_pwrctl_ctl =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_PWRCTL_CTL);
	msm_host->regs_restore.hc_38_3a =
		sdhci_readl(host, SDHCI_SIGNAL_ENABLE);
	msm_host->regs_restore.hc_34_36 = sdhci_readl(host, SDHCI_INT_ENABLE);
	msm_host->regs_restore.hc_28_2a = sdhci_readl(host, SDHCI_HOST_CONTROL);
	msm_host->regs_restore.vendor_caps_0 = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC_CAPABILITIES0);
	msm_host->regs_restore.hc_caps_1 =
		sdhci_readl(host, SDHCI_CAPABILITIES_1);
	msm_host->regs_restore.testbus_config = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_TESTBUS_CONFIG);
	msm_host->regs_restore.dll_config =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_CONFIG);
	msm_host->regs_restore.dll_config2 = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_DLL_CONFIG_2);
	msm_host->regs_restore.dll_config =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_CONFIG);
	msm_host->regs_restore.dll_config2 = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_DLL_CONFIG_2);
	msm_host->regs_restore.dll_config3 = readl_relaxed(
		host->ioaddr + msm_host_offset->CORE_DLL_CONFIG_3);
	msm_host->regs_restore.dll_usr_ctl =
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_USR_CTL);
...
static void sdhci_msm_registers_restore(struct sdhci_host *host)
{
	...
	writel_relaxed(msm_host->regs_restore.vendor_func,
		       host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC);
	...
	sdhci_writel(host, msm_host->regs_restore.hc_38_3a,
		     SDHCI_SIGNAL_ENABLE);
	sdhci_writel(host, msm_host->regs_restore.hc_34_36, SDHCI_INT_ENABLE);
	sdhci_writel(host, msm_host->regs_restore.hc_28_2a, SDHCI_HOST_CONTROL);
	...
	writel_relaxed(msm_host->regs_restore.testbus_config,
		       host->ioaddr + msm_host_offset->CORE_TESTBUS_CONFIG);
...
static void sdhci_msm_toggle_fifo_write_clk(struct sdhci_host *host)
{
	...
	if (msm_host->tuning_done ||
	    (card && mmc_card_strobe(card) && msm_host->enhanced_strobe)) {
		/*
...
static void sdhci_msm_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if ((curr_ios.timing == MMC_TIMING_UHS_DDR50) ||
		 ...
		 */
		ddr_clock = clock * 2;
		sup_clock = sdhci_msm_get_sup_clk_rate(host, ddr_clock);
	}
	...
	if (curr_ios.timing == MMC_TIMING_MMC_HS400) {
		 ...
		 * register
		 */
		if ((msm_host->tuning_done || (card && mmc_card_strobe(card) &&
		...
		if (!host->mmc->ios.old_rate && !msm_host->use_cdclp533) {
			...
			if (card && mmc_card_strobe(card) &&
				...
				rc = readl_poll_timeout(
					...
					dll_lock,
					(dll_lock &
					 (CORE_DLL_LOCK | CORE_DDR_DLL_LOCK)),
					10, 1000);
			}
			if (rc == -ETIMEDOUT)
		...
		writel_relaxed(
			(readl_relaxed(host->ioaddr +
				       msm_host_offset->CORE_VENDOR_SPEC) &
			 ~CORE_HC_SELECT_IN_EN & ~CORE_HC_SELECT_IN_MASK),
			host->ioaddr + msm_host_offset->CORE_VENDOR_SPEC);
	}
	/*
	 * SDHC has core_mem and hc_mem device memory and these memory
	...
	if (sup_clock != msm_host->clk_rate) {
		...
		if (!IS_ERR(msm_host->bus_aggr_clk) &&
			...
			bus_clk_rate = sdhci_msm_get_bus_aggr_clk_rate(
				host, sup_clock);
			if (bus_clk_rate >= 0) {
		...
		if (rc)
			pr_err("%s: %s: Failed to set pinctrl drive type for clock rate %u (%d)\n",
			       mmc_hostname(host->mmc), __func__, clock, rc);
					...
					unsigned int uhs)
{
	...
	if ((uhs == MMC_TIMING_MMC_HS400) || (uhs == MMC_TIMING_MMC_HS200) ||
	...
	else if ((uhs == MMC_TIMING_UHS_DDR50) || (uhs == MMC_TIMING_MMC_DDR52))
...
static void sdhci_msm_cache_debug_data(struct sdhci_host *host)
{
	...
	memcpy(&cached_data->copy_mmc, msm_host->mmc, sizeof(struct mmc_host));
	if (msm_host->mmc->card)
		memcpy(&cached_data->copy_card, msm_host->mmc->card,
		       sizeof(struct mmc_card));
	memcpy(&cached_data->copy_host, host, sizeof(struct sdhci_host));
}
...
static void sdhci_msm_cqe_dump_debug_ram(struct sdhci_host *host)
{
	...
	pr_err(DRV_NAME
	       ": Debug RAM wrap-around: 0x%08x | Debug RAM overlap: 0x%08x\n",
...
void sdhci_msm_dump_fsm_history(struct sdhci_host *host)
{
	...
	for (sel_fsm = 0; sel_fsm <= MAX_FSM; sel_fsm++) {
		writel_relaxed(sel_fsm,
			       host->ioaddr + SDCC_DEBUG_FSM_TRACE_CFG_REG);
	...
	writel_relaxed(DUMMY,
		       host->ioaddr + SDCC_DEBUG_FSM_TRACE_FIFO_FLUSH_REG);

	/* Exit from Auto recovery disable to move FSMs to idle state */
	writel_relaxed(DUMMY, host->ioaddr + SDCC_DEBUG_ERROR_STATE_EXIT_REG);
}
...
void sdhci_msm_dump_iib(struct sdhci_host *host)
{
	...
	for (iter = 0; iter < 8; iter++)
		pr_err("0x%08x\n",
		       readl_relaxed(host->ioaddr + SDCC_DEBUG_IIB_REG +
				     (iter * 4)));
}
...
void sdhci_msm_dump_vendor_regs(struct sdhci_host *host)
{
	...
	mmc_log_string(host->mmc, "Data cnt: 0x%08x | Fifo cnt: 0x%08x\n",
		       sdhci_msm_readl_relaxed(
			       host, msm_host_offset->CORE_MCI_DATA_CNT),
		       sdhci_msm_readl_relaxed(
			       host, msm_host_offset->CORE_MCI_FIFO_CNT));
	...
	pr_info("DLL sts: 0x%08x | DLL cfg:  0x%08x | DLL cfg2: 0x%08x\n",
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_STATUS),
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_DLL_CONFIG),
		sdhci_msm_readl_relaxed(host,
					msm_host_offset->CORE_DLL_CONFIG_2));
	...
	pr_info("SDCC ver: 0x%08x | Vndr adma err : addr0: 0x%08x addr1: 0x%08x\n",
		readl_relaxed(host->ioaddr + msm_host_offset->CORE_MCI_VERSION),
		readl_relaxed(host->ioaddr +
	...
	for (tbsel2 = 0; tbsel2 < 7; tbsel2++) {
		for (tbsel = 0; tbsel < 8; tbsel++) {
			...
			test_bus_val =
				(tbsel2
				 << msm_host_offset->CORE_TESTBUS_SEL2_BIT) |
				tbsel | msm_host_offset->CORE_TESTBUS_ENA;
			sdhci_msm_writel_relaxed(
				test_bus_val, host,
				msm_host_offset->CORE_TESTBUS_CONFIG);
			debug_reg[index++] = sdhci_msm_readl_relaxed(
				host, msm_host_offset->CORE_SDCC_DEBUG_REG);
		}
	}
	for (i = 0; i < MAX_TEST_BUS; i = i + 4)
		pr_info(" Test bus[%d to %d]: 0x%08x 0x%08x 0x%08x 0x%08x\n", i,
			i + 3, debug_reg[i], debug_reg[i + 1], debug_reg[i + 2],
			debug_reg[i + 3]);
}
...
void sdhci_msm_reset_workaround(struct sdhci_host *host, u32 enable)
{
	...
	if (enable) {
		writel_relaxed(vendor_func2 | HC_SW_RST_REQ,
			       host->ioaddr +
				       msm_host_offset->CORE_VENDOR_SPEC_FUNC2);
		timeout = 10000;
		while (readl_relaxed(host->ioaddr +
			...
			if (timeout == 0) {
				 ...
				 * AXI bus.
				 */
				vendor_func2 = readl_relaxed(
					host->ioaddr +
					msm_host_offset->CORE_VENDOR_SPEC_FUNC2);
				writel_relaxed(
					vendor_func2 | HC_SW_RST_WAIT_IDLE_DIS,
					host->ioaddr +
						msm_host_offset
...
{
	struct sdhci_msm_pm_qos_irq *pm_qos_irq = container_of(
		work, struct sdhci_msm_pm_qos_irq, unvote_work.work);
...
void sdhci_msm_pm_qos_irq_vote(struct sdhci_host *host)
{
	...
	if (msm_host->pm_qos_irq.latency ==
		    latency->latency[host->power_policy] &&
	    counter > 1)
		return;
...
static ssize_t sdhci_msm_pm_qos_irq_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
...
static ssize_t sdhci_msm_pm_qos_irq_enable_show(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
...
static ssize_t sdhci_msm_pm_qos_irq_enable_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
					   ...
					   char *buf)
{
	...
	for (i = 0; i < nr_groups; i++) {
		...
		offset += snprintf(
			...
			atomic_read(&group->counter), group->latency);
	}
...
static ssize_t
sdhci_msm_pm_qos_group_enable_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
...
{
	struct sdhci_msm_pm_qos_group *group = container_of(
		work, struct sdhci_msm_pm_qos_group, unvote_work.work);
			       ...
			       struct sdhci_msm_pm_qos_latency *latency)
{
	...
	msm_host->pm_qos =
		kcalloc(nr_groups, sizeof(*msm_host->pm_qos), GFP_KERNEL);
	if (!msm_host->pm_qos)
		return;
	...
	for (i = 0; i < nr_groups; i++) {
		...
		pr_info("%s (): voted for group #%d (mask=0x%lx) latency=%d\n",
			__func__, i, group->req.cpus_affine.bits[0],
			group->latency);
	}
	...
	if (ret)
		dev_err(&msm_host->pdev->dev,
			"%s: fail to create pm_qos_group_status_attr (%d)\n",
			__func__, ret);
	...
	if (ret)
		dev_err(&msm_host->pdev->dev,
			"%s: fail to create pm_qos_group_enable_attr (%d)\n",
			__func__, ret);
}
			      ...
			      struct mmc_request *mmc_req)
{
	...
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	int prev_group =
		sdhci_msm_get_cpu_group(msm_host, msm_host->pm_qos_prev_cpu);
	...
	if (group != prev_group && prev_group >= 0) {
		sdhci_msm_pm_qos_cpu_unvote(host, msm_host->pm_qos_prev_cpu,
					    false);
		prev_group = -1; /* make sure to vote for new group */
	}

	if (prev_group < 0) {
		sdhci_msm_pm_qos_cpu_vote(
			host, msm_host->pdata->pm_qos_data.latency, cpu);
				      ...
				      struct sdhci_host *host)
{
	...
	version = sdhci_msm_readl_relaxed(host,
					  msm_host_offset->CORE_MCI_VERSION);
	major = (version & CORE_VERSION_MAJOR_MASK) >> CORE_VERSION_MAJOR_SHIFT;
	minor = version & CORE_VERSION_TARGET_MASK;
	 ...
	 */
	if ((major == 1) &&
	    ((minor == 0x42) || (minor == 0x46) || (minor == 0x49) ||
	     (minor == 0x4D) || (minor >= 0x6b)))
	...
	if ((major == 1) &&
	    ((minor == 0x6e) || (minor == 0x71) || (minor == 0x72)))
...
static bool sdhci_msm_is_bootdevice(struct device *dev)
{
	if (strnstr(saved_command_line,
		    "androidboot.bootdevice=", strlen(saved_command_line))) {
...
static void sdhci_msm_select_bus_mode(struct sdhci_host *host)
{
	...
	if (bus_mode) {
		if (bus_mode == SELECT_HS400) {
			...
			msm_host->pdata->caps2 &= ~(MMC_CAP2_HS200);
			msm_host->pdata->caps &=
				~(MMC_CAP_3_3V_DDR | MMC_CAP_1_8V_DDR |
				  MMC_CAP_1_2V_DDR);
			msm_host->mmc->clk_scaling.lower_bus_speed_mode &=
				~(MMC_SCALING_LOWER_DDR52_MODE);
		}
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		msm_host->pdata =
			sdhci_msm_populate_pdata(&pdev->dev, msm_host);
		if (!msm_host->pdata) {
	...
	core_memres =
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "core_mem");
	if (!msm_host->mci_removed) {
		if (!core_memres) {
		...
		msm_host->core_mem =
			devm_ioremap(&pdev->dev, core_memres->start,
	...
	tlmm_memres =
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "tlmm_mem");
	if (tlmm_memres) {
	...
	if (!msm_host->mci_removed) {
		...
		writel_relaxed(
			readl_relaxed(msm_host->core_mem + CORE_HC_MODE) |
				FF_CLK_SW_RST_DIS,
	 ...
	 */
	irq_status = sdhci_msm_readl_relaxed(
		host, msm_host_offset->CORE_PWRCTL_STATUS);
	...
	msm_host->minor =
		IPCAT_MINOR_MASK(readl_relaxed(host->ioaddr + SDCC_IP_CATALOG));
	...
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
		...
		if (ret) {
			dev_err(&pdev->dev,
				"%s: Failed to request card detection IRQ %d\n",
				__func__, ret);
			goto vreg_deinit;
		}
	}
	...
	if (sdhci_is_valid_gpio_wakeup_int(msm_host)) {
		...
		if (ret) {
			dev_err(&pdev->dev,
				"%s: request sdiowakeup IRQ %d: failed: %d\n",
	...
	msm_host->pdata->testbus_trigger_irq =
		platform_get_irq_byname(pdev, "tb_trig_irq");
	if (sdhci_is_valid_gpio_testbus_trigger_int(msm_host)) {
		...
		if (ret) {
			dev_err(&pdev->dev,
				"%s: request tb_trig IRQ %d: failed: %d\n",
	...
	if (msm_host->minor >= 2) {
		...
		ret = device_create_file(&pdev->dev, &msm_host->mask_and_match);
		if (ret) {
...
static int sdhci_msm_remove(struct platform_device *pdev)
{
	...
	int i;
	int dead =
		(readl_relaxed(host->ioaddr + SDHCI_INT_STATUS) == 0xffffffff);
...
skip_clk_gating:
	trace_sdhci_msm_runtime_suspend(
		mmc_hostname(host->mmc), 0,
...
defer_enable_host_irq:
	...
	trace_sdhci_msm_runtime_resume(
		mmc_hostname(host->mmc), 0,
		ktime_to_us(ktime_sub(ktime_get(), start)));
	return 0;
}
