...
struct pmu {
	...
	u32 events_across_hotplug : 1, reserved : 31;
...
#define PERF_EF_UPDATE 0x04 /* update the counter when stopping */
	 ...
	 */
	void *(*setup_aux)(struct perf_event *event, void **pages, int nr_pages,
			   bool overwrite);
	/* optional */
...
extern struct perf_event *perf_event_create_kernel_counter(
...
int perf_event_read_local(struct perf_event *event, u64 *value, u64 *enabled,
			  u64 *running);
extern u64 perf_event_read_value(struct perf_event *event, u64 *enabled,
				 u64 *running);
...
#define PERF_MEM_NA                                                            \
	(PERF_MEM_S(OP, NA) | PERF_MEM_S(LVL, NA) | PERF_MEM_S(SNOOP, NA) |    \
	 PERF_MEM_S(LOCK, NA) | PERF_MEM_S(TLB, NA))
...
static inline void perf_arch_fetch_caller_regs(struct pt_regs *regs,
					       unsigned long ip)
{
}
#endif
...
static __always_inline void perf_sw_event(u32 event_id, u64 nr,
...
{
	if (static_key_false(
		    &perf_swevent_enabled[PERF_COUNT_SW_CPU_MIGRATIONS]))
		return true;
	return false;
}
...
extern int
perf_register_guest_info_callbacks(struct perf_guest_info_callbacks *callbacks);
...
extern void perf_callchain_user(struct perf_callchain_entry_ctx *entry,
				struct pt_regs *regs);
...
static inline int
perf_callchain_store_context(struct perf_callchain_entry_ctx *ctx, u64 ip)
{
...
static inline void *perf_aux_output_begin(struct perf_output_handle *handle,
					  struct perf_event *event)
{
	return NULL;
}
static inline void perf_aux_output_end(struct perf_output_handle *handle,
				       unsigned long size)
{
}
static inline int perf_aux_output_skip(struct perf_output_handle *handle,
				       unsigned long size)
{
	return -EINVAL;
}
...
static inline const struct perf_event_attr *
